...
After starting the wallet shell (native of WASM) and starting a local rusty-cryptix Cryptixd node (for example with `--testnet`; the UTXO index is enabled by default), you should perform the following actions:
```
network testnet
server localhost
connect
