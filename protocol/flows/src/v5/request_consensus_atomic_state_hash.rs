...
use cryptix_p2p_lib::{
    ...
    pb::{cryptixd_message::Payload, ConsensusAtomicStateHashMessage},
...
pub struct RequestConsensusAtomicStateHashFlow {
    ctx: FlowContext,
    router: Arc<Router>,
...
impl Flow for RequestConsensusAtomicStateHashFlow {
    ...
    async fn start(&mut self) -> Result<(), ProtocolError> {
        loop {
            let (request, request_id) = dequeue_with_request_id!(self.incoming_route, Payload::RequestConsensusAtomicStateHash)?;
            if request_id == BLANK_ROUTE_ID {
                return Err(ProtocolError::MisbehavingPeer("RequestConsensusAtomicStateHash missing non-zero request id".to_string()));
            }
            let Some(block_hash) = request.block_hash else {
                return Err(ProtocolError::Other("RequestConsensusAtomicStateHash missing block hash"));
            };
            ...
            let state_hash = loop {
                if !self.ctx.is_ibd_running() {
                    ...
                    if local_anchor_matches {
                        match session.async_get_atomic_state_hash(block_hash).await {
            ...
            self.router
                .enqueue(make_response!(
                    Payload::ConsensusAtomicStateHash,
                    ConsensusAtomicStateHashMessage { block_hash: Some(block_hash.into()), state_hash, has_state, anchor_daa_score },
                    request_id
                ))
                .await?;
        }
...
impl RequestConsensusAtomicStateHashFlow {
